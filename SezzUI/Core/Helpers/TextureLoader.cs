...
namespace DelvUI.Helpers
{
    public static class TextureLoader
    {
        public static TextureWrap? LoadTexture(string path, bool manualLoad)
        {
            if (!manualLoad)
            {
                TexFile? iconFile = SezzUI.Plugin.DataManager.GetFile<TexFile>(path);
                if (iconFile != null)
                {
                    return SezzUI.Plugin.UiBuilder.LoadImageRaw(iconFile.GetRgbaImageData(), iconFile.Header.Width, iconFile.Header.Height, 4);
                }
            }
        ...
        private static unsafe TextureWrap? ManuallyLoadTexture(string path)
        {
            try
            {
                ...
                return SezzUI.Plugin.UiBuilder.LoadImageRaw(GetRgbaImageData(imageData), Header.Width, Header.Height, 4);
            }
            catch
            {
