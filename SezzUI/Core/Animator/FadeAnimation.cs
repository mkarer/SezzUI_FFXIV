...
namespace SezzUI.Animator
{
	...
	public class FadeAnimation : BaseAnimation
	{
		...
		private FadeDirection _defaultDirection;
		private FadeDirection _currentDirection; // TODO: Use for reversing animation?
		...
		public override void Update()
		{
            if (_isPlaying && _ticksStart != null && Data != null)
            {
				...
				if (timeElapsed > StartDelay && timeElapsed <= StartDelay + Duration)
				{
					...
					float fadeProgress = Math.Min(1, Math.Max(0, (float)timeElapsedAnimating / (float)Duration));
