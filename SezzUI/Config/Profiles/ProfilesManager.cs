...
namespace SezzUI.Config.Profiles
{
	public class ProfilesManager
	{
		...
		[JsonIgnore]
		private static string ProfilesPath => Path.Combine(ConfigurationManager.Instance.ConfigDirectory, "Profiles");
		...
		public void SaveCurrentProfile()
		{
			...
			{
				Logger.Error(ex, "SaveCurrentProfile", "Error saving profile: " + ex.Message);
			}
		}
		...
		public bool LoadCurrentProfile()
		{
			...
			{
				Logger.Error(ex, "LoadCurrentProfile", "Error loading profile: " + ex.Message);
			}
		...
		private string? SwitchToProfile(string profile, bool save = true)
		{
			...
			{
				Logger.Error(ex, "SwitchToProfile", "Error saving profile: " + ex.Message);
				return "Couldn't load profile \"" + profile + "\"!";
			}
		...
		private string? CloneProfile(string profileName, string srcPath, string newProfileName, string dstPath)
		{
			...
			{
				Logger.Error(ex, "CloneProfile", "Error cloning profile: " + ex.Message);
		...
		private string? RenameCurrentProfile(string newProfileName)
		{
			...
			{
				Logger.Error(ex, "RenameCurrentProfile", "Error renaming profile: " + ex.Message);
		...
		private string? Import(string newProfileName, string importString)
		{
			...
			{
				Logger.Error(ex, "Import", "Error importing profile: " + ex.Message);
		...
		private void ImportFromFile(string newProfileName)
		{
			...
			Action<bool, string> callback = (finished, path) =>
			{
				...
				{
					Logger.Error(ex, "ImportFromFile", "Error reading import file: " + ex.Message);
		...
		private void ExportToFile(string newProfileName)
		{
			...
			Action<bool, string> callback = (finished, path) =>
			{
				...
				{
					Logger.Error(ex, "ExportToFile", "Error copying file: " + ex.Message);
		...
		private string? DeleteProfile(string profileName)
		{
			...
			{
				Logger.Error(ex, "DeleteProfile", "Error deleting profile: " + ex.Message);
