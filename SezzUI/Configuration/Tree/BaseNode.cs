	...
	public void Draw(float alpha)
	{
		...
		ImGui.BeginGroup(); // Middle section
		{
			ImGui.BeginGroup(); // Left
			{
				...
				if (banner != null && banner.Handle != IntPtr.Zero)
				{
					ImGui.SetCursorPos(new(10, 12));
					ImGui.Image(banner.Handle, banner.Size);
				}
				else
				{
		...
		ImGui.BeginGroup();
		{
			...
			FontAwesomeIcon hudVisibilityIcon = Singletons.Get<ConfigurationManager>().ShowHUD ? FontAwesomeIcon.ToggleOn : FontAwesomeIcon.ToggleOff;
			if (ImGuiHelper.FontAwesomeIconButton("Toggle Visibility", hudVisibilityIcon, buttonSize))
			{
				Singletons.Get<ConfigurationManager>().ShowHUD = !Singletons.Get<ConfigurationManager>().ShowHUD;
			}

			// Button: Lock/Unlock HUD
			string hudLockText = (Singletons.Get<ConfigurationManager>().LockHUD ? "Unlock" : "Lock") + " HUD";
			FontAwesomeIcon hudLockIcon = Singletons.Get<ConfigurationManager>().LockHUD ? FontAwesomeIcon.ArrowsAlt : FontAwesomeIcon.Lock;

			ImGui.SameLine();
			ImGui.SetCursorPosX(ImGui.GetCursorPosX() + 2);
			if (ImGuiHelper.FontAwesomeIconButton(hudLockText, hudLockIcon, buttonSize))
			{
				Singletons.Get<ConfigurationManager>().LockHUD = !Singletons.Get<ConfigurationManager>().LockHUD;
			}
		...
		{
			Singletons.Get<ConfigurationManager>().CloseConfigWindow();
		}
